...
interface HeaderProps {
  rename: Dispatch<SetStateAction<boolean>>,
  deleteTab: (tb: Tab) => void,
  addTodo: (tb: boolean) => void,
  activeTab: Tab | undefined
}

export default function TabHeader({ rename, deleteTab, addTodo, activeTab = undefined }: HeaderProps) {
  ...
  return (
    <header id="header-todo" className="flex justify-between items-center mb-6">
      <h2 style={{ color: activeTab.color }} className="text-2xl font-bold">{activeTab.title}</h2>
